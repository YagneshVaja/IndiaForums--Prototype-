...
function MoviePosterCardImpl({ movie, onPress }: Props) {
    ...
    >
      ...
      <Text style={styles.ratingLine} numberOfLines={1}>
        ...
        <Text style={styles.ratingNumber}>
          {movie.criticRatingCount > 0 ? movie.criticRatingCount : 0}
        </Text>
      </Text>
      <Text style={styles.ratingLine} numberOfLines={1}>
        <Text style={styles.ratingLabel}>User Rating </Text>
        <Text style={styles.ratingNumber}>
          {movie.audienceRatingCount > 0 ? movie.audienceRatingCount : 0}
        </Text>
      </Text>
    </Pressable>
  );
...
const MoviePosterCard = React.memo(MoviePosterCardImpl);
...
function makeStyles(c: ThemeColors) {
  return StyleSheet.create({
    ...
    fallbackEmoji: { fontSize: 30, marginBottom: 6, opacity: 0.9 },
    fallbackTitle: {
      fontSize: 12.5,
      fontWeight: '800',
      color: '#FFFFFF',
